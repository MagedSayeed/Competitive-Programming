/** Remember that:
 *  - This is **Manacher's Algorithm** to find the **Longest Palindromic Substring** in O(n) time.
 *  - It handles both even- and odd-length palindromes by inserting '#' between characters.
 *    For example, "abba" becomes "#a#b#b#a#"
 * 
 *  - `dp[i]` stores the length (diameter) of the longest palindrome centered at index `i` in the preprocessed string.
 *  - `left` and `right` track the boundaries of the current known palindrome.
 *  - `idx` stores the center of the longest palindrome found so far.
 * 
 *  - After processing, extract the longest palindrome from the original string by skipping '#' characters.
 * 
 * Use Cases:
 *  - Efficiently find the longest palindromic substring in any string.
 *  - Useful in text analysis, DNA sequence analysis, and palindrome-based problems in CP.
 * 
 * Time Complexity: O(n)
