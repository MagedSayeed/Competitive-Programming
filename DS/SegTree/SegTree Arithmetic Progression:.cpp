...
#define RT ((x<<1)|1)
    ...
    void update(int l, int r, lazy_node to_add,
                int x = 1, int lx = 0, int rx = -1) {
        if (rx == -1)rx = sz - 1;
        if (lazy_status[x])
            propagate(x, lx, rx, lazy[x]);
        if (lx > rx || lx > r || rx < l) return;
        if (l <= lx && rx <= r) {
            int diff = lx - l;
            int a = to_add.a, d = to_add.d;
            lazy_node curr =
        ...
        update(l, r, to_add, LF, lx, md);
        update(l, r, to_add, RT, md + 1, rx);
